...
import YodleeClient from './YodleeClient-V1.1';
...
import type {
  AuthPayload$Cobrand,
  AuthPayload$CobrandAndUser,
  ProviderQuery as YodleeProviderQuery,
} from './YodleeClient-V1.1';
import type { ID } from 'common/types/core';
import type { Provider as YodleeProvider } from 'common/types/yodlee-v1.1';
...
function initialize(): void {
  ...
  initializeAuth(cobrandLogin, cobrandPassword, cobrandLocale);
}
...
type GenFetchProviders = (
  ID,
  YodleeProviderQuery,
) => Promise<Array<YodleeProvider>>;
const genFetchProviders: GenFetchProviders = overrideClientAsyncMethod(
  'genFetchProviders',
);
...
type AuthData = {|
  dateOfLastVerification: Date,
  payload: AuthPayload$CobrandAndUser,
|};

const MILLIS_BEFORE_CHECKING_USER_AUTH = 10000;
const MILLIS_BEFORE_CHECKING_COBRAND_AUTH = 60000;
const userToAuth: { [userID: ID]: AuthData } = {};
let cobrandAuth: AuthPayload$Cobrand | null = null;

function initializeAuth(cobrandLogin, cobrandPassword, cobrandLocale): void {}

async function genFetchUserAuth(
  userID: ID,
): Promise<AuthPayload$CobrandAndUser> {
  const cobrandAuth = await genFetchCobrandAuth();

  const authData = userToAuth[userID];

  // Check if the user already has an authentication payload.
  if (authData) {
    const now = new Date();
    if (
      now - authData.dateOfLastVerification <
      MILLIS_BEFORE_CHECKING_USER_AUTH
    ) {
      return authData.payload;
    }

    // Check if the user's auth payload is still valid.
    try {
      await YodleeClient.genFetchUser(authData.payload);
      authData.dateOfLastVerification = new Date();
      return authData.payload;
    } catch (error) {
      INFO('YODLEE', 'Fetching user failed. Assuming authentication timed out');
    }
  }

  // Auth data either does not exist or is expired. Login user.
  const credentials = await YodleeCredentialsFetcher.genNullthrows(userID);
  const payload = await YodleeClient.genLoginUser(
    credentials.loginName,
    credentials.password,
  );
}

async function genFetchCobrandAuth(): Promise<AuthPayload$Cobrand> {
  if (cobrandAuth) {
    // Verify cobrand auth has not yet expired.
  }
}
...
async function genCheckAndRefreshYodleeUserSession(userID: ID): Promise<void> {
  ...
  const cobrandAuth = await genWaitForCobrandLogin;
