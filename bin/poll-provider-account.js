...
if (!providerAccountID) {
  console.log(chalk.red('You must provide argument --user-id=<userID>'));
  process.exit(1);
}
...
// Continuously ping provider account for updates.
promise = promise.then(check);

// Handle errors.
promise.catch(error => {
  console.log(chalk.red('ERROR!'));
  const description = error.errorMessage || error.error_message || error.message || error.toString();
...
function logProviderAccount(providerAccount) {
  console.log('\nPROVIDER ACCOUNT FETCHED');
  console.log(`Status\t${providerAccount.refreshInfo.status}`);
  console.log(`Add. Status\t${providerAccount.refreshInfo.additionalStatus}`);
  console.log('--------------------------------\n');
}
...
function check() {
  return yodleeClient
    .genProviderAccount(yodleeUserSession, providerAccountID)
    .then(providerAccount => {
      logProviderAccount(providerAccount);
      return wait();
    });
}
